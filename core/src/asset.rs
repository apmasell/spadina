...
use crate::net::server::AssetError;
...
impl<S: AsRef<str> + Clone, B: AsRef<[u8]>> Asset<S, B> {
  pub fn create_realm_template(&self, children: &BTreeMap<String, impl AsRef<Asset<S, B>>>) -> Result<GenericControllerTemplate, Option<Vec<S>>> {
    todo!()
  }
