...
impl<S: AsRef<str>, B> HostEvent<S, B> {
    ...
    <R as Referencer<S>>::Output<'a>: AsRef<str>,
  {
    match self {
      ...
      HostEvent::PlayerRequest { player, is_admin, request } => {
        HostEvent::PlayerRequest { player: player.reference(reference), is_admin: *is_admin, request: reference.convert(request) }
      }
    }
  }
    ...
    <C as Converter<S>>::Output: AsRef<str>,
  {
    match self {
      ...
      HostEvent::PlayerRequest { player, is_admin, request } => {
        HostEvent::PlayerRequest { player: player.convert(converter), is_admin, request: converter.convert(request) }
      }
    }
  }
}
