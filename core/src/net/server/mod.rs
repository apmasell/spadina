...
pub enum ClientRequest<S: AsRef<str> + Eq + std::hash::Hash + Ord, B> {
  ...
  CalendarLocationAdd {
    id: u32,
    location: location::target::LocalTarget<S>,
  },
...
pub enum ClientResponse<S: AsRef<str> + Eq + std::hash::Hash + Ord, B> {
  ...
  Bookmarks {
    bookmarks: std::collections::HashSet<Resource<S>>,
  },
  ...
  LocationAvatars {
    players: std::collections::HashMap<player::PlayerIdentifier<S>, avatar::Avatar>,
  },
...
impl<S: AsRef<str> + Eq + std::hash::Hash + Ord + Serialize, B: Serialize> From<ClientRequest<S, B>> for Message {
  fn from(value: ClientRequest<S, B>) -> Self {
    Message::Binary(rmp_serde::to_vec_named(&value).expect("Failed to serialize message"))
  }
}
