  ...
  C::Output: Serialize,
{
  ...
  fn process(&mut self, input: ControllerInput<Self::Input, &str>) -> Vec<ControllerOutput<Self::Output>> {
    match input {
      ...
      ControllerInput::Input { player, player_id, player_kind, request } => match rmp_serde::from_slice(&request) {
        Err(_) => return Vec::new(),
        Ok(request) => self.0.process(ControllerInput::Input { player, player_id, player_kind, request }),
      },
