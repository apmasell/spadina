...
pub enum Update {
  ...
  BoolAmbientLightColor(bevy::render::color::Color, bevy::render::color::Color, puzzleverse_core::asset::Transition),
  BoolAmbientLightIntensity(f64, f64, puzzleverse_core::asset::Transition),
  BoolChangeMeshMaterial(
    bevy::ecs::entity::Entity,
    bevy::asset::Handle<bevy::pbr::StandardMaterial>,
    bevy::asset::Handle<bevy::pbr::StandardMaterial>,
    puzzleverse_core::asset::Transition,
  ),
  BoolChangeLightColor(bevy::ecs::entity::Entity, bevy::render::color::Color, bevy::render::color::Color, puzzleverse_core::asset::Transition),
  BoolChangeLightIntensity(bevy::ecs::entity::Entity, f64, f64, puzzleverse_core::asset::Transition),
  BoolVisibility(bevy::ecs::entity::Entity),
  NumShared(std::sync::Arc<std::sync::atomic::AtomicU32>),
  NumAmbientLightColor(bevy::render::color::Color, Vec<bevy::render::color::Color>, puzzleverse_core::asset::Transition),
  NumAmbientLightIntensity(f64, Vec<f64>, puzzleverse_core::asset::Transition),
  ...
  NumChangeLightColor(bevy::ecs::entity::Entity, bevy::render::color::Color, Vec<bevy::render::color::Color>, puzzleverse_core::asset::Transition),
  NumChangeLightIntensity(bevy::ecs::entity::Entity, f64, Vec<f64>, puzzleverse_core::asset::Transition),
}

impl Update {
  pub fn process(&self, value: &puzzleverse_core::PropertyValue, world: &mut bevy::ecs::world::World) {
    match (value, self) {
      (puzzleverse_core::PropertyValue::Bool(value), Update::BoolShared(atomic)) => {
        atomic.store(*value, std::sync::atomic::Ordering::Relaxed);
      }
      (puzzleverse_core::PropertyValue::Bool(value), Update::BoolAmbientLightColor(when_true, when_false, _)) => {
        if let Some(light) = world.get_resource_mut::<bevy::pbr::AmbientLight>() {
          light.color = (if *value { when_true } else { when_false }).clone();
        }
      }
      (puzzleverse_core::PropertyValue::Bool(value), Update::BoolAmbientLightIntensity(when_true, when_false, _)) => {
        if let Some(light) = world.get_resource_mut::<bevy::pbr::AmbientLight>() {
          light.brightness = *(if *value { when_true } else { when_false }) as f32 * crate::convert::MAX_ILLUMINATION;
        }
      }
      (puzzleverse_core::PropertyValue::Bool(value), Update::BoolChangeMeshMaterial(entity, when_true, when_false, _)) => {
      ...
      (puzzleverse_core::PropertyValue::Bool(value), Update::BoolChangeLightColor(entity, when_true, when_false, _)) => {
        if let Some(light) = world.get_entity_mut(*entity).map(|e| e.get_mut::<bevy::pbr::PointLight>()).flatten() {
          light.color = (if *value { when_true } else { when_false }).clone();
        }
      }
      (puzzleverse_core::PropertyValue::Bool(value), Update::BoolChangeLightIntensity(entity, when_true, when_false, _)) => {
        if let Some(light) = world.get_entity_mut(*entity).map(|e| e.get_mut::<bevy::pbr::PointLight>()).flatten() {
          light.intensity = *(if *value { when_true } else { when_false }) as f32 * crate::convert::MAX_ILLUMINATION;
        }
      }
      (puzzleverse_core::PropertyValue::Bool(value), Update::BoolVisibility(entity)) => {
      ...
      (puzzleverse_core::PropertyValue::Num(value), Update::NumShared(atomic)) => {
        atomic.store(*value, std::sync::atomic::Ordering::Relaxed);
      }
      (puzzleverse_core::PropertyValue::Num(value), Update::NumAmbientLightColor(default, colors, _)) => {
        if let Some(light) = world.get_resource_mut::<bevy::pbr::AmbientLight>() {
          light.color = colors.get(*value as usize).unwrap_or(default).clone();
        }
      }
      (puzzleverse_core::PropertyValue::Num(value), Update::NumAmbientLightIntensity(default, intensities, _)) => {
        if let Some(light) = world.get_resource_mut::<bevy::pbr::AmbientLight>() {
          light.brightness = *intensities.get(*value as usize).unwrap_or(default) as f32 * crate::convert::MAX_ILLUMINATION;
        }
      }
      (puzzleverse_core::PropertyValue::Num(value), Update::NumChangeMeshMaterial(entity, default, materials, _)) => {
      ...
      (puzzleverse_core::PropertyValue::Num(value), Update::NumChangeLightColor(entity, default, colors, _)) => {
        if let Some(light) = world.get_entity_mut(*entity).map(|e| e.get_mut::<bevy::pbr::PointLight>()).flatten() {
          light.color = colors.get(*value as usize).unwrap_or(default).clone();
        }
      }
      (puzzleverse_core::PropertyValue::Num(value), Update::NumChangeLightIntensity(entity, default, intensities, _)) => {
        if let Some(light) = world.get_entity_mut(*entity).map(|e| e.get_mut::<bevy::pbr::PointLight>()).flatten() {
