...
struct CacheStates {
  ...
  >,
  account_locks: cache::KeyCache<spadina_core::net::server::ClientRequest<String>, spadina_core::access::AccountLockState>,
  ...
  direct_messages: cache::KeyCache<spadina_core::net::server::ClientRequest<String>, Vec<DirectMessage>>,
  failed_operations: Shared<Vec<FailedOperation>>,
  id: std::sync::Arc<std::sync::atomic::AtomicI32>,
  invitations: mutator::Mutator<()>,
  known_realms: cache::KeyCache<spadina_core::net::server::ClientRequest<String>, Vec<spadina_core::realm::RealmDirectoryEntry<String>>>,
  known_servers: cache::Cache<std::collections::BTreeSet<RemoteServer>>,
  location_access: cache::Cache<(Vec<spadina_core::access::AccessControl<spadina_core::access::OnlineAccess>>, spadina_core::access::OnlineAccess)>,
  outstanding: Shared<std::collections::HashMap<i32, InflightRequest>>,
  player_location: cache::KeyCache<spadina_core::net::server::ClientRequest<String>, spadina_core::player::OnlineState<String>>,
  public_keys: cache::Cache<std::collections::HashSet<spadina_core::net::server::auth::PublicKey<String>>>,
...
impl<S: location::LocationState> ServerConnection<S> {
  pub fn new<A: spadina_core::asset_store::AssetStore + 'static>(asset_store: A, runtime: &tokio::runtime::Runtime) -> Self {
    ...
    runtime.spawn(async move {
      ...
      loop {
        enum Event<S> {
          ...
          Server(spadina_core::net::server::ClientResponse<String>),
          ServerError(String),
          ServerQuit,
          Ignore,
        ...
        match event {
              ...
              match request {
                ...
                location::LocationEvent::PullAsset(principal) => match asset_store.pull(&principal).await {
                  ...
                  Err(spadina_core::asset_store::LoadError::Unknown) => {
                    ...
                    state.deliver(spadina_core::net::server::ClientRequest::AssetPull { principal }).await;
                  }
                  Err(e) => {
                    state
                      .deliver(spadina_core::net::server::ClientRequest::LocationChange {
                        location: spadina_core::location::protocol::LocationRequest::NoWhere,
                      })
                      .await;
          ...
          Event::Server(response) => match response {
            spadina_core::net::server::ClientResponse::AccountLockChange { id, name, result } => {
            ...
            spadina_core::net::server::ClientResponse::AccountLockStatus { name, status } => {
              cache_state.account_locks.update(name, |s| *s = status);
            }
            spadina_core::net::server::ClientResponse::AccessChange { id, response } => {
              if let Some(operation) = cache_state.outstanding.lock().unwrap().remove(&id) {
            ...
            spadina_core::net::server::ClientResponse::Announcements { announcements } => cache_state.announcements.update(|a| {
              a.clear();
              a.extend(announcements);
            }),
            spadina_core::net::server::ClientResponse::AnnouncementUpdate { id, result } => {
            ...
            spadina_core::net::server::ClientResponse::AssetCreationSucceeded { id, hash } => {
              cache_state.asset_upload.finish(mutator::AssetUpload(id), Ok(hash))
            }
            spadina_core::net::server::ClientResponse::AssetCreationFailed { id, error } => {
              cache_state.asset_upload.finish(mutator::AssetUpload(id), Err(error))
            }
            spadina_core::net::server::ClientResponse::Asset { principal, asset } => {
              ...
              for dispatch in asset_dispatch.remove(&principal).into_iter().flatten() {
                match dispatch {
                  AssetDispatch::ConsensualEmote(id, player) => match emote_cache.get(&principal, &asset_store).await {
                    ...
                    _ => {
                      state.deliver(spadina_core::net::server::ClientRequest::ConsensualEmoteResponse { id, ok: false }).await;
                    }
                  },
            ...
            spadina_core::net::server::ClientResponse::AssetUnavailable { principal } => {
              for dispatch in asset_dispatch.remove(&principal).into_iter().flatten() {
                match dispatch {
            ...
            spadina_core::net::server::ClientResponse::AvatarCurrent { avatar } => {
              cache_state.avatar.update(|a| *a = avatar);
            }
            spadina_core::net::server::ClientResponse::AvatarUpdate { id, success } => {
              if let Some(InflightRequest { created, operation, .. }) = cache_state.outstanding.lock().unwrap().remove(&id) {
                if !success {
                  cache_state.failed_operations.lock().unwrap().push(FailedOperation {
                    created,
            ...
            spadina_core::net::server::ClientResponse::Bookmarks { bookmarks } => cache_state.bookmarks.update(|b| {
              *b = bookmarks;
            }),
            spadina_core::net::server::ClientResponse::BookmarkUpdate { id, success } => {
              if let Some(InflightRequest { created, operation, .. }) = cache_state.outstanding.lock().unwrap().remove(&id) {
                if !success {
                  cache_state.failed_operations.lock().unwrap().push(FailedOperation {
                    created,
            ...
            spadina_core::net::server::ClientResponse::Calendar { id } => cache_state.calendar_id.update(|i| {
              *i = id;
            }),
            spadina_core::net::server::ClientResponse::CalendarUpdate { id, result } => {
            ...
            spadina_core::net::server::ClientResponse::CalendarLocationChange { id, success } => {
            ...
            spadina_core::net::server::ClientResponse::CalendarLocationList { locations: realms } => cache_state.calendar_realms.update(|r| {
              *r = realms;
            }),
            spadina_core::net::server::ClientResponse::Disconnect => {
              *screen.lock().unwrap() = ScreenState::Quit;
              state = connection::ConnectionState::Idle;
            }
            spadina_core::net::server::ClientResponse::ConsensualEmoteRequest { id, emote, player } => {
              match emote_cache.get(&emote, &asset_store).await {
                emote_cache::EmoteResult::Emote(emote) => current_location.request_emote(id, emote, player),
                emote_cache::EmoteResult::Missing => {
                  state.deliver(spadina_core::net::server::ClientRequest::AssetPull { principal: emote.clone() }).await;
            ...
            spadina_core::net::server::ClientResponse::FollowRequest { id, player } => {
              current_location.request_follow(id, player);
            }
            spadina_core::net::server::ClientResponse::LocationMessagePosted { sender, body, timestamp } => {
              current_location.message_posted(sender, body, timestamp);
            }
            spadina_core::net::server::ClientResponse::LocationMessages { messages, from, to } => {
              current_location.messages(messages, from, to);
            }

            spadina_core::net::server::ClientResponse::CurrentAccess { target, rules: acls, default } => {
              cache_state.access.update(target, |control| *control = (acls, default));
            }
            spadina_core::net::server::ClientResponse::DirectMessageReceipt { id, status } => {
            ...
            spadina_core::net::server::ClientResponse::DirectMessageStats { stats, last_login } => cache_state.direct_message_stats.update(|c| {
              c.stats = stats;
              c.last_login = last_login;
            }),
            spadina_core::net::server::ClientResponse::DirectMessages { player, messages } => cache_state.direct_messages.update(player, |m| {
            ...
            spadina_core::net::server::ClientResponse::InviteSuccess { id, url } => cache_state.invitations.finish(mutator::Invitation(id), Ok(url)),
            spadina_core::net::server::ClientResponse::InviteFailure { id, error } => {
              cache_state.invitations.finish(mutator::Invitation(id), Err(error))
            }
            spadina_core::net::server::ClientResponse::PublicKeys { keys } => cache_state.public_keys.update(|k| {
              k.clear();
              k.extend(keys);
            }),
            spadina_core::net::server::ClientResponse::PublicKeyUpdate { id, result } => {
              if let Some(InflightRequest { created, operation, .. }) = cache_state.outstanding.lock().unwrap().remove(&id) {
                if result != spadina_core::UpdateResult::Success {
                  cache_state.failed_operations.lock().unwrap().push(FailedOperation {
                    created,
            ...
            spadina_core::net::server::ClientResponse::LocationChange { location } => {
              asset_dispatch.clear();
              *screen.lock().unwrap() = ScreenState::InTransit;
              let (new_location, new_screen_state) = match location {
                spadina_core::location::protocol::LocationResponse::Resolving => (location::Location::NoWhere, ScreenState::InTransit),
                spadina_core::location::protocol::LocationResponse::Realm { owner, server, name, asset, in_directory, seed, settings } => {
                ...
                spadina_core::location::protocol::LocationResponse::Hosting => {
                  let (location, world) = location::Location::new_host();
                  (location, ScreenState::InWorld(world))
                }
                spadina_core::location::protocol::LocationResponse::Guest { host } => {
                  let (location, world) = location::Location::new_guest(host);
                  (location, ScreenState::InWorld(world))
                }
                spadina_core::location::protocol::LocationResponse::NoWhere => (location::Location::NoWhere, ScreenState::Lost(None)),
                ...
                spadina_core::location::protocol::LocationResponse::PermissionDenied => {
                  (location::Location::NoWhere, ScreenState::Lost(Some(std::borrow::Cow::Borrowed("Not allowed to go to this realm."))))
                }
                spadina_core::location::protocol::LocationResponse::ResolutionFailed => {
                  (location::Location::NoWhere, ScreenState::Lost(Some(std::borrow::Cow::Borrowed("Could not find realm."))))
                }
                spadina_core::location::protocol::LocationResponse::MissingCapabilities { capabilities } => (
                  location::Location::NoWhere,
            ...
            spadina_core::net::server::ClientResponse::LocationDelete { id, result } => cache_state.realm_deletion.finish(
            ...
            spadina_core::net::server::ClientResponse::LocationsAvailable { display, realms } => cache_state.known_realms.update(display, |v| {
              v.clear();
              v.extend(realms)
            }),
            spadina_core::net::server::ClientResponse::Peers { mut peers } => {
              peers.sort();
            ...
            spadina_core::net::server::ClientResponse::PeersBanned { bans } => {
              cache_state.banned_servers.update(|known_bans| {
                known_bans.clear();
                known_bans.extend(bans);
              });
            }
            spadina_core::net::server::ClientResponse::PeersBannedUpdate { id, result } => {
              if let Some(InflightRequest { created, operation, .. }) = cache_state.outstanding.lock().unwrap().remove(&id) {
                if result != spadina_core::UpdateResult::Success {
                  cache_state.failed_operations.lock().unwrap().push(FailedOperation {
                    created,
            ...
            spadina_core::net::server::ClientResponse::PlayerOnlineState { player, state } => {
              cache_state.player_location.update(player, |s| *s = state);
            }
            spadina_core::net::server::ClientResponse::InLocation { response } => current_location.handle_realm(response),
            spadina_core::net::server::ClientResponse::CurrentAccessOnline { rules, default } => {
              cache_state.location_access.update(|access| *access = (rules, default))
            }
            spadina_core::net::server::ClientResponse::LocationAvatars { players } => current_location.update_avatars(players),
            spadina_core::net::server::ClientResponse::NoOperation => (),
            spadina_core::net::server::ClientResponse::ToHost { event } => current_location.handle_host_event(event),
            spadina_core::net::server::ClientResponse::FromHost { response } => current_location.handle_host(response),
            spadina_core::net::server::ClientResponse::PlayerReset { id, result } => {
              if let Some(InflightRequest { created, operation, .. }) = cache_state.outstanding.lock().unwrap().remove(&id) {
                if result != spadina_core::UpdateResult::Success {
                  cache_state.failed_operations.lock().unwrap().push(FailedOperation {
                    created,
            ...
            spadina_core::net::server::ClientResponse::TrainAdd { id, result } => {
              if let Some(InflightRequest { created, operation, .. }) = cache_state.outstanding.lock().unwrap().remove(&id) {
                if result != spadina_core::net::server::TrainAddResult::Success {
                  cache_state.failed_operations.lock().unwrap().push(FailedOperation {
                    created,
                    failed: chrono::Utc::now(),
  ...
  pub fn account_locks(&self) -> cache::KeyCacheRef<'_, spadina_core::net::server::ClientRequest<String>, spadina_core::access::AccountLockState, S> {
  ...
  pub fn direct_messages(&self) -> cache::KeyCacheRef<'_, spadina_core::net::server::ClientRequest<String>, Vec<DirectMessage>, S> {
  ...
  pub fn known_realms(
    &self,
  ) -> cache::KeyCacheRef<'_, spadina_core::net::server::ClientRequest<String>, Vec<spadina_core::realm::RealmDirectoryEntry<String>>, S> {
  ...
  pub fn player_location(
    &self,
  ) -> cache::KeyCacheRef<'_, spadina_core::net::server::ClientRequest<String>, spadina_core::player::OnlineState<String>, S> {
    self.cache_state.player_location.capture(self)
  }
  pub fn public_keys(&self) -> cache::CacheRef<'_, std::collections::HashSet<spadina_core::net::server::auth::PublicKey<String>>, S> {
