...
impl<K: Clone + Send + Sync + Ord + Eq + Unpin + 'static, V: OutputMapper<K>> StreamableMap for BTreeMap<K, V> {
  type Entry<'a> = btree_map::OccupiedEntry<'a, K, V> where Self:'a,K:'a, V:'a;
  type IterMut<'a> = btree_map::IterMut<'a, K, V > where Self:'a,K: 'a, V:'a;
  type Key = K;
  type Value = V;
...
impl<K: Clone + Send + Sync + Ord + Eq + Hash + Unpin + 'static, V: OutputMapper<K>> StreamableMap for HashMap<K, V> {
  type Entry<'a> = hash_map::OccupiedEntry<'a, K, V> where Self:'a,K:'a, V:'a;
  type IterMut<'a> = hash_map::IterMut<'a, K, V>where Self:'a,K:'a, V:'a;
  type Key = K;
  type Value = V;
