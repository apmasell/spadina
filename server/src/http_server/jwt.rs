...
pub fn encode_jwt_redirect<C: serde::Serialize>(claim: &C, auth: &AccessManagement) -> hyper::http::Result<Response<Full<Bytes>>> {
  match encode_jwt(claim, auth) {
    Ok(token) => {
      Response::builder().status(StatusCode::TEMPORARY_REDIRECT).header(LOCATION, format!("spadina-login:{}", token)).body(Default::default())
    }
