...
async fn start() -> Result<(), Box<dyn Error + Send + Sync>> {
  ...
  let database = database::Database::new(&configuration.database_url);
  ...
  start_message_cleaner_task(&directory.access_management, database.clone());
...
fn start_message_cleaner_task(auth: &AccessManagement, database: database::Database) {
