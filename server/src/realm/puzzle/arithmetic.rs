...
struct Arithmetic {
  left: u32,
  right: u32,
  operation: spadina_core::asset::puzzle::ArithmeticOperation,
}

pub struct ArithmeticAsset(pub spadina_core::asset::puzzle::ArithmeticOperation);

impl crate::realm::puzzle::PuzzleAsset for ArithmeticAsset {
  fn create(
    self: Box<Self>,
    _: &chrono::DateTime<chrono::Utc>,
    _: &mut std::collections::BTreeMap<crate::shstr::ShStr, super::RadioSharedState>,
  ) -> Box<dyn crate::realm::puzzle::PuzzlePiece> {
    Box::new(Arithmetic { left: 0, right: 0, operation: self.0 }) as Box<dyn crate::realm::puzzle::PuzzlePiece>
  }
  fn load<'a>(
    self: Box<Self>,
    ...
    Ok(Box::new(Arithmetic { left, right, operation: self.0 }) as Box<dyn crate::realm::puzzle::PuzzlePiece>)
  }
}

impl crate::realm::puzzle::PuzzlePiece for Arithmetic {
  fn accept(
    self: &mut Self,
    ...
    if update && old_state != new_state {
      vec![crate::realm::puzzle::OutputEvent::Event(
        ...
        spadina_core::asset::rules::PieceValue::Num(new_state),
      )]
    } else {
      vec![]
    }
