    ...
    match incoming {
      ...
      Incoming::Delayed(location::LocationEvent::Redirect(redirect)) => {
        let location = match redirect {
          ...
          UnresolvedTarget::Absolute { descriptor, owner, server } => {
            ...
            let target = LocalTarget { owner, descriptor };
            if server.as_ref() == directory.access_management.server_name.as_ref() {
              directory.join_location(target, join_request).await
            } else {
              directory.join_location_on_peer(target, server, join_request).await
            }
            location
          }
      ...
      Incoming::External(ClientRequest::AccessSetDirectMessage { rules, default, id }) => {
        ...
        let response = Outgoing::Send(
          ClientResponse::<String, Vec<u8>>::AccessChange {
            id,
            response: self.message_acl.mutate(|settings| {
      ...
      Incoming::External(ClientRequest::AccessSetDefault { id, rules, default }) => {
        vec![Outgoing::Send(
          ClientResponse::<String, Vec<u8>>::AccessChange {
            id,
            response: self.default_location_acl.mutate(|settings| {
              settings.default = default;
              settings.rules = rules;
              UpdateResult::Success
            }),
      ...
      Incoming::External(ClientRequest::AccessSetLocationBulk { id, selection, rules, default }) => {
        ...
        vec![Outgoing::Send(ClientResponse::<String, Vec<u8>>::AccessChange { id, response }.into())]
      }
      Incoming::External(ClientRequest::AccessOnlineSet { id, rules, default }) => {
        vec![Outgoing::Send(
          ClientResponse::<String, Vec<u8>>::AccessChange {
            id,
            response: self.online_acl.mutate(|settings| {
              settings.default = default;
              settings.rules = rules;
              UpdateResult::Success
            }),
      ...
      Incoming::External(ClientRequest::CalendarReset { id, player }) => {
        let player = match player.filter(|player| player.as_str() != &*self.name) {
          None => Ok((PlayerReference::Id(self.db_id), true)),
          Some(player) => {
            if is_superuser || directory.access_management.accounts.is_administrator(&self.name).await {
              Ok((PlayerReference::Name(player), false))
            } else {
              Err(UpdateResult::NotAllowed)
            }
          }
        };

        let (result, reset) = match player {
          Ok((player, should_send)) => match database.calendar_reset(player) {
            Ok(id) => (UpdateResult::Success, if should_send { Some(id) } else { None }),
            Err(e) => {
              eprintln!("Failed to reset calendar link for {}: {}", &self.name, e);
              (UpdateResult::InternalError, None)
            }
          },
          Err(e) => (e, None),
        };
        let message = Outgoing::Send(ClientResponse::<String, _>::CalendarUpdate { id, result, reset: reset.as_ref() }.into());
      ...
      Incoming::External(ClientRequest::CalendarLocationAdd { id, location }) => {
        let success = match database.calendar_add(self.db_id, &location) {
          Ok(_) => true,
          Err(e) => {
      ...
      Incoming::External(ClientRequest::CalendarLocationRemove { id, location }) => {
        let success = match database.calendar_rm(self.db_id, &location) {
          Ok(_) => true,
          Err(e) => {
      ...
      Incoming::External(ClientRequest::PlayerReset { id, player, reset }) => {
        let result = if is_superuser || directory.access_management.accounts.is_administrator(&self.name).await {
      ...
      Incoming::External(ClientRequest::LocationsList { id, source, timeout }) => {
        ...
        match incremental_search::ReifiedSearch::convert(source, &self.name, self.db_id, &directory.access_management.server_name) {
          ...
          incremental_search::ReifiedSearch::Calendar => {
            location_search::local_results(recipient, database.calendar_list(self.db_id, &directory.access_management.server_name), directory)
          }
      ...
      Incoming::External(ClientRequest::PeerBanClear { id, bans }) => {
      ...
      Incoming::External(ClientRequest::PeerBanSet { id, bans }) => {
        let result = if is_superuser || directory.access_management.accounts.is_administrator(&self.name).await {
          directory
            .access_management
            .banned_peers
            .write("client_set_ban", |banned| {
              banned.extend(bans.into_iter().filter_map(|ban| ban.clean()));
              Some(true)
            })
            .await
        } else {
