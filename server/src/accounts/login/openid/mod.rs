...
use spadina_core::net::server::auth::{AuthScheme, OpenIdConnectInformation};
        ...
        (&Method::GET, OIDC_AUTH_START_PATH) => match query_parameter("player", &req) {
          ...
          Some(player) => match self.provider.client_for(&*player).await {
            ...
            Some(client) => {
              ...
              match serde_json::to_vec(&OpenIdConnectInformation { authorization_url: auth_url.as_str(), request_id: &request_id }) {
                Ok(body) => AuthResult::Page(Response::builder().status(http::StatusCode::OK).body(body.into())),
                Err(e) => {
                  eprintln!("Failed to serialize OpenID: {}", e);
                  AuthResult::Failure
                }
              }
            }
