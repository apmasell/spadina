use crate::accounts::db_auth::schema_oidc::authoidc::dsl as auth_oidc_schema;
...
impl DatabaseOpenIdConnect {
  pub fn new_postgres(database_url: String, clients: BTreeMap<String, OpenIdClient>) -> Result<Self, Box<dyn Error + Send + Sync>> {
    let pool = {
    ...
    Ok(DatabaseOpenIdConnect { pool: DatabaseBackend::Postgresql(pool), clients })
  }
  #[cfg(feature = "mysql")]
  pub fn new_mysql(database_url: String, clients: BTreeMap<String, OpenIdClient>) -> Result<Self, Box<dyn Error + Send + Sync>> {
    let pool = {
    ...
    let mut db_connection = pool.get()?;
    db_connection.run_pending_migrations(OIDC_MIGRATIONS)?;
    Ok(DatabaseOpenIdConnect { pool: DatabaseBackend::MySql(pool), clients })
  }
}
