...
pub(crate) trait AuthProvider: Send + Sync {
  ...
  async fn invite(&self, server_name: &str, connection: &crate::database::Database) -> Result<String, spadina_core::communication::InvitationError>;
  ...
  async fn handle(self: &Self, req: http::Request<hyper::Body>, connection: &crate::database::Database) -> AuthResult;
}
...
impl AuthConfiguration {
  ...
  pub(crate) async fn load(self, server_name: &str) -> Result<Box<dyn AuthProvider>, String> {
    match self {
      AuthConfiguration::DatabaseOTPs => crate::auth::db_otp::new(),
      ...
      AuthConfiguration::MultipleOpenIdConnect { providers, registration } => {
        let mut clients = std::collections::BTreeMap::new();
        for configuration in providers {
          let (client, issuer, name) =
            create_oidc_client(configuration.provider, configuration.client_id, configuration.client_secret, server_name).await?;
          clients.insert(issuer, (name, client));
        }
        db_oidc_multiple::new(clients, registration)
      }
      AuthConfiguration::OpenIdConnect { client_id, client_secret, provider, registration } => {
        db_oidc::new(create_oidc_client(provider, client_id, client_secret, server_name).await?.0, registration)
      }
      ...
      AuthOpenIdConnectEndpoint::Microsoft { tenant } => (
        format!(
          ...
          match tenant {
            ...
            None => std::borrow::Cow::Borrowed("9188040d-6c67-4c5b-b112-36a304b66dad"),
          }
        ),
        "Microsoft".to_owned(),
      ),
...
#[async_trait::async_trait]
pub(crate) trait Password: Send + Sync {
  /// Check if the username and password provided are valid
  async fn check(self: &Self, username: &str, password: &str, connection: &crate::database::Database) -> bool;
  /// Check if an account is locked.
  async fn is_locked(&self, username: &str, connection: &crate::database::Database) -> spadina_core::access::AccountLockState;
  /// Lock (or unlock) a user account
  async fn lock(&self, username: &str, locked: bool, connection: &crate::database::Database) -> spadina_core::UpdateResult;
...
where
  T: Password,
{
  fn scheme(self: &Self) -> spadina_core::auth::AuthScheme {
    spadina_core::auth::AuthScheme::Password
  }
  ...
  async fn is_locked(&self, username: &str, connection: &crate::database::Database) -> spadina_core::access::AccountLockState {
    Password::is_locked(self, username, connection).await
  }
  async fn lock(&self, username: &str, locked: bool, connection: &crate::database::Database) -> spadina_core::UpdateResult {
    Password::lock(self, username, locked, connection).await
  }

  async fn handle(self: &Self, req: http::Request<hyper::Body>, connection: &crate::database::Database) -> AuthResult {
    use bytes::Buf;
    match (req.method(), req.uri().path()) {
      (&http::Method::POST, spadina_core::net::PASSWORD_AUTH_PATH) => match hyper::body::aggregate(req).await {
        Err(e) => {
          eprintln!("Failed to aggregate body: {}", e);
          AuthResult::Failure
        }
        Ok(whole_body) => match serde_json::from_reader::<_, spadina_core::auth::PasswordRequest<String>>(whole_body.reader()) {
          Err(e) => AuthResult::Page(http::Response::builder().status(http::StatusCode::BAD_REQUEST).body(e.to_string().into())),
          Ok(data) => {
            if self.check(&data.username, &data.password, connection).await {
...
#[async_trait::async_trait]
pub(crate) trait OTPStore: Send + Sync {
  /// Check if an account is locked.
  async fn is_locked(&self, username: &str, connection: &crate::database::Database) -> spadina_core::access::AccountLockState;
  /// Lock (or unlock) a user account
  async fn lock(&self, username: &str, locked: bool, connection: &crate::database::Database) -> spadina_core::UpdateResult;
  ...
  T: OTPStore,
{
  async fn check(self: &Self, username: &str, password: &str, connection: &crate::database::Database) -> bool {
    match password.parse::<u32>() {
      Ok(code) => self.secret(&username, connection).await.drain(..).any(|secret| {
  ...
  async fn is_locked(&self, username: &str, connection: &crate::database::Database) -> spadina_core::access::AccountLockState {
    OTPStore::is_locked(self, username, connection).await
  }

  async fn lock(&self, username: &str, locked: bool, connection: &crate::database::Database) -> spadina_core::UpdateResult {
    OTPStore::lock(self, username, locked, connection).await
  }
}
/// Create an authentication provider that deals with unencrypted usernames and passwords
#[async_trait::async_trait]
pub(crate) trait OpenIdConnectProvider: Send + Sync {
  type RegistrationTarget: serde::Serialize + serde::de::DeserializeOwned + Send + Sync;
  /// Check if the username and password provided are valid
  async fn client(&self, username: &str, connection: &crate::database::Database) -> Option<&openidconnect::core::CoreClient>;
  ...
  async fn register(
    &self,
    username: &str,
    selector: &Self::RegistrationTarget,
    subject: &str,
    connection: &crate::database::Database,
  ) -> Result<(), String>;
  fn registration(&self) -> OpenIdRegistration;
  async fn registration_client(
    &self,
    selector: &Self::RegistrationTarget,
    connection: &crate::database::Database,
  ) -> Option<&openidconnect::core::CoreClient>;
  ...
  async fn validate(&self, username: &str, subject: &str, connection: &crate::database::Database) -> bool;
}
...
impl<T: OpenIdConnectProvider + 'static> OpenIdConnect<T> {
  pub(crate) fn new(provider: T) -> Result<Box<dyn AuthProvider>, String> {
    Ok(Box::new(OpenIdConnect {
  ...
  T: OpenIdConnectProvider,
{
  ...
  async fn invite(&self, server_name: &str, _: &crate::database::Database) -> Result<String, spadina_core::communication::InvitationError> {
    match self.provider.registration() {
      OpenIdRegistration::Closed => Err(spadina_core::communication::InvitationError::Closed),
      OpenIdRegistration::Open => Ok(format!("https://{}/register", server_name)),
      OpenIdRegistration::Invite => match &self.invitations {
        Some(invitations) => {
          ...
          if let Err(e) = openssl::rand::rand_bytes(&mut invitation) {
            ...
            return Err(spadina_core::communication::InvitationError::InternalError);
          }
          let invitation = base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(invitation);
          let url = format!("https://{}/register?invitation={}", server_name, &invitation);
          invitations.lock().await.insert(invitation);
          Ok(url)
        }
        None => Err(spadina_core::communication::InvitationError::Closed),
      },
    }
  }

  async fn is_locked(&self, username: &str, connection: &crate::database::Database) -> spadina_core::access::AccountLockState {
    self.provider.is_locked(username, connection).await
  }

  async fn lock(&self, username: &str, locked: bool, connection: &crate::database::Database) -> spadina_core::UpdateResult {
    self.provider.lock(username, locked, connection).await
  }
  async fn handle(self: &Self, req: http::Request<hyper::Body>, connection: &crate::database::Database) -> AuthResult {
    ...
    match (req.method(), req.uri().path()) {
      (&http::Method::GET, spadina_core::net::OIDC_AUTH_START_PATH) => match query_parameter("player", &req) {
        None => AuthResult::Failure,
        Some(player) => match self.provider.client(&*player, connection).await {
          None => AuthResult::Failure,
          Some(client) => {
            use sha3::Digest;
            ...
            AuthResult::Page(
              http::Response::builder().status(http::StatusCode::OK).body(
                serde_json::to_vec(&spadina_core::auth::OpenIdConnectInformation { authorization_url: auth_url.to_string(), request_id })
                  .unwrap()
                  .into(),
              ),
            )
      ...
      (&http::Method::GET, spadina_core::net::OIDC_AUTH_FINISH_PATH) => match query_parameter("request_id", &req) {
        Some(request_id) => {
          let state = {
        ...
        (Some(csrf_token), Some(code)) => match {
          ...
          Some(info) => {
            let client = match &info.callback {
              OpenIdCallback::Login { player, .. } => self.provider.client(player, connection).await,
              OpenIdCallback::Register { client_type, .. } => self.provider.registration_client(client_type, connection).await,
            };
            match client {
              None => AuthResult::Failure,
              Some(client) => {
                  ...
                  Ok(auth) => match auth.id_token().map(|id| id.claims(&client.id_token_verifier(), &nonce)) {
                    ...
                    match info.callback {
                      OpenIdCallback::Login {player, sender} => {
                        let (ok, message) = if self.provider.validate(&player, &*id_token.subject(), connection).await {
                      ...
                      OpenIdCallback::Register{ player, invitation, client_type} =>
                        match self.provider.register(&player, &client_type, &*id_token.subject(), connection).await {
                        Ok(()) => {
                    ...
                    None =>  std::borrow::Cow::Borrowed("The Spadina server has be connected to a non-OpenID Connect-enable OAuth server. Contact your server administrator. If you are the server administrator, choose a different OpenID server or adjust it to enable OpenID Connect."),
                  },
          ...
          match serde_urlencoded::from_reader::<OpenIdRegistrationRequest<T::RegistrationTarget>, _>(whole_body.reader()) {
            Ok(request) => {
              ...
              AuthResult::Page(if is_in_progress {
                ...
                } else if self.provider.is_available(&request.player, connection).await {
                  match self.provider.registration_client(&request.client_type, connection).await {
                    Some(client) => {
                      let (pkce_challenge, pkce_verifier) = openidconnect::PkceCodeChallenge::new_random_sha256();
                      let (auth_url, csrf_token, nonce) = client
                        .authorize_url(
