use puzzleverse_core::asset_store::AssetStore;
#[derive(Debug)]
enum Command {
  Install,
}
...
fn main() {
  ...
  let mut args = vec![];
  {
    ...
    ap.set_description("Puzzleverse On-Disk Asset Tool");
  ...
  let asset_store = puzzleverse_core::asset_store::FileSystemStore::new(std::path::Path::new(&directory), [4, 4, 8].iter().cloned());
